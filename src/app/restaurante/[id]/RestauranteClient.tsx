...
export function RestauranteClient({ id }: Props) {
  ...
  if (!restaurante) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-8 text-center">
        ...
        <p className="text-stone-700 font-semibold">Negocio no encontrado</p>
        <button
          onClick={() => router.push("/")}
    ...
    <div className="flex flex-col min-h-screen pb-32 bg-white">
      ...
      <header className="sticky top-0 z-40 bg-white shadow-sm">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="font-bold text-stone-900 text-sm leading-tight truncate">{restaurante.nombre}</h1>
      ...
      <div className={`${config.bg} px-4 py-5 border-b border-stone-100`}>
        <div className="flex items-center gap-4">
          <div className={`relative w-20 h-20 rounded-2xl overflow-hidden ${config.bg} ring-2 ring-white shadow-md shrink-0`}>
            <Image src={restaurante.logo_url} alt={`Logo de ${restaurante.nombre}`} fill className="object-cover" sizes="80px" />
          </div>
          <div className="flex-1">
            <h2 className="font-extrabold text-stone-900 text-base leading-tight">{restaurante.nombre}</h2>
            <p className="text-stone-500 text-xs mt-1 leading-relaxed">{restaurante.descripcion}</p>
            <span className={`inline-flex items-center gap-1 mt-2 text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-white/80 ${config.text}`}>
      ...
      {hayItemsDeOtroRestaurante && (
        <div className="mx-4 mt-3 bg-amber-50 border border-amber-200 rounded-2xl p-3 flex items-start gap-2 text-xs text-amber-700">
          <span className="text-base shrink-0">⚠️</span>
          <p>Tienes productos de <strong>{carrito.restaurante_nombre}</strong> en tu carrito. Al agregar aquí se vaciará el anterior.</p>
        </div>
      )}
        ...
        {Array.from(productosPorCategoria.entries()).map(([categoria, prods]) => (
          <section key={categoria} className="mb-7">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-bold text-stone-500 uppercase tracking-widest px-2">{categoria}</span>
              <div className="h-px flex-1 bg-stone-100" />
            </div>
...
function TarjetaProducto({ producto, cantidad, onAgregar, onIncrementar, onDecrementar }: TarjetaProductoProps) {
  ...
  return (
    <div className={`relative bg-white rounded-2xl border transition-all duration-200
                     ${enCarrito ? "border-orange-200 shadow-md shadow-orange-50" : "border-stone-100 shadow-sm"}
                     ${!producto.disponible ? "opacity-60" : ""}`}>
      {enCarrito && (
        <div className="absolute -top-1.5 -right-1.5 bg-orange-500 text-white text-[10px] font-bold rounded-full w-5 h-5 flex items-center justify-center shadow">
          {cantidad}
        </div>
      )}
      <div className="flex items-center gap-3 p-3">
        <div className="relative w-[68px] h-[68px] rounded-xl overflow-hidden bg-stone-100 shrink-0">
          {producto.imagen_url ? (
            ...
            <div className="w-full h-full flex items-center justify-center text-2xl bg-gradient-to-br from-stone-100 to-stone-200">🍽️</div>
          )}
        </div>
        <div className="flex-1 min-w-0">
          ...
          {!producto.disponible && <span className="text-[11px] text-stone-400 font-medium">No disponible</span>}
        </div>
        <div className="shrink-0">
          {!producto.disponible ? (
            ...
            <button onClick={onAgregar}
              className="bg-orange-500 hover:bg-orange-600 active:bg-orange-700 text-white rounded-xl px-3 py-2 text-xs font-bold transition-colors shadow-sm shadow-orange-200 whitespace-nowrap"
            ...
            <div className="flex items-center gap-1.5">
              <button onClick={onDecrementar}
                className="w-8 h-8 rounded-xl bg-stone-100 hover:bg-stone-200 active:bg-stone-300 text-stone-700 font-bold text-base flex items-center justify-center transition-colors"
              ...
              <button onClick={onIncrementar}
                className="w-8 h-8 rounded-xl bg-orange-500 hover:bg-orange-600 active:bg-orange-700 text-white font-bold text-base flex items-center justify-center transition-colors"
                aria-label="Agregar uno más">+</button>
            </div>
          )}
        </div>
      </div>
